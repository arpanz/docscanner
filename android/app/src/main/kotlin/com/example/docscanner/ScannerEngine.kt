...
class ScannerEngine(
    ...
    // Callbacks
    var onFrameAnalyzed: ((Bitmap, List<Double>) -> Unit)? = null
    var onEdgeDetected: ((List<Double>) -> Unit)? = null
                        ...
                        .also { analysis ->
                            analysis.setAnalyzer(cameraExecutor) { imageProxy ->
                                try {
                                    ...
                                    // Send corners to Flutter for overlay drawing
                                    corners?.let {
                                        val cornerList = flattenCorners(it)
                                        onEdgeDetected?.invoke(cornerList)
                                        onFrameAnalyzed?.invoke(bitmap, cornerList)
                                    }
                                } catch (e: Exception) {
    ...
    fun stopCamera() {
        camera?.let {
            it.cameraInfo.cameraState // Just to check if valid
        }
        try {
    ...
    private fun approxQuad(contour: MatOfPoint): MatOfPoint2f? {
        ...
        if (approx.total() == 4L) {
            val points = MatOfPoint2f(*approx.toArray().map { p -> Point(p.x.toDouble(), p.y.toDouble()) }.toTypedArray())
    ...
    private fun applyMagicColor(mat: Mat): Bitmap {
        ...
        val lab = Mat()
        Imgproc.cvtColor(mat, lab, Imgproc.COLOR_RGBA2Lab)
        ...
        Imgproc.cvtColor(lab, result, Imgproc.COLOR_Lab2RGBA)
        
        // Boost saturation slightly
        val hsv = Mat()
        Imgproc.cvtColor(result, hsv, Imgproc.COLOR_RGBA2HSV_FULL)
        val hsvChannels = ArrayList<Mat>()
        Core.split(hsv, hsvChannels)
        
        // Increase saturation (S channel) by 20%
        Core.multiply(hsvChannels[1], Mat(hsv.size(), CvType.CV_32FC1, Scalar(1.2)), hsvChannels[1])
        Core.merge(hsvChannels, hsv)
        
        Imgproc.cvtColor(hsv, result, Imgproc.COLOR_HSV2RGBA_FULL)
    ...
    private fun ImageProxy.toBitmap(): Bitmap {
        ...
        return BitmapFactory.decodeByteArray(bytes, 0, bytes.size).also {
            // Rotate if needed based on camera rotation
